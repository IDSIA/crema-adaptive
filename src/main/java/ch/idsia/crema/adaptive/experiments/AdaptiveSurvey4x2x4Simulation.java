...
import ch.idsia.crema.adaptive.experiments.model.imprecise.Credal4x2x4;
import ch.idsia.crema.adaptive.experiments.model.precise.Bayesian4x2x4;
 ...
 */
public class AdaptiveSurvey4x2x4Simulation {
	...
	static final int N_QUESTIONS = 4;
	...
	public static void main(String[] args) throws Exception {
		...
		final Bayesian4x2x4 bayesian4x2x4 = new Bayesian4x2x4(N_QUESTIONS);
		...
		final TIntIntMap[] samples = bns.samples(bayesian4x2x4.getModel(), N_STUDENTS);
		...
		final List<Student<BayesianFactor>> students = IntStream.range(0, N_STUDENTS)
				.mapToObj(id -> new Student<BayesianFactor>(id, samples[id], bayesian4x2x4.skills))
				.collect(Collectors.toList());

		final String path = "output/MultiSkill4x2x4/";

		final ExperimentSuite[] experiments = {
				new ExperimentSuite(
						path, "Bayesian4x2x4", "non-adaptive", students,
						() -> new Teacher<>(new Bayesian4x2x4(N_QUESTIONS), new ScoringFunctionRandom(0)).setPersist(new PersistBayesian()),
						2, 3
				),
				new ExperimentSuite(
						path, "Bayesian4x2x4", "bayesian-adaptive-entropy", students,
						() -> new Teacher<>(new Bayesian4x2x4(N_QUESTIONS), new ScoringFunctionExpectedEntropy()).setPersist(new PersistBayesian()),
						0, 1, 3
				),
				new ExperimentSuite(
						path, "Bayesian4x2x4", "bayesian-adaptive-mode", students,
						() -> new Teacher<>(new Bayesian4x2x4(N_QUESTIONS), new ScoringFunctionBayesianMode()).setPersist(new PersistBayesian()),
						0, 1, 3
				),
				new ExperimentSuite(
						path, "Credal4x2x4", "credal-adaptive-entropy", students,
						() -> new Teacher<>(new Credal4x2x4(N_QUESTIONS).setEps(.01), new ScoringFunctionUpperExpectedEntropy(), new StoppingConditionCredalMeanEntropy(.1)).setPersist(new PersistCredal()),
						0, 1, 3
				),
				new ExperimentSuite(
						path, "Credal4x2x4", "credal-adaptive-mode", students,
						() -> new Teacher<>(new Credal4x2x4(N_QUESTIONS).setEps(.01), new ScoringFunctionCredalMode(), new StoppingConditionCredalMeanEntropy(.1)).setPersist(new PersistCredal()),
						0, 1, 3
				),
		};
